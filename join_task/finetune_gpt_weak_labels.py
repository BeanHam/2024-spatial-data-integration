...
if __name__ == '__main__':
    ...
    train_response = client.files.create(
        file=open(path.join(args.formatted_data_dir, f'train_{args.metric_name}_{args.metric_value}_weak.jsonl'), "rb"),
        purpose="fine-tune"
    )
    val_response = client.files.create(
        file=open(path.join(args.formatted_data_dir, f'val_{args.metric_name}_{args.metric_value}_weak.jsonl'), "rb"),
        purpose="fine-tune"
    )
