...
if __name__ == '__main__':
    ...
    for metric_value in args.metric_values:
        ...
        def generate_weak_labels(example):    
            if args.metric_name == 'degree':
                weak_label=int(example['min_angle']<=metric_value)
            elif args.metric_name == 'distance':
                weak_label=int(example['euc_dist']>=metric_value)        
        ...
        with open(path.join(args.formatted_data_dir, f'train_{args.metric_name}_{metric_value}_weak.jsonl'), 'w') as f:
            f.write(train_formatted)
        with open(path.join(args.formatted_data_dir, f'val_{args.metric_name}_{metric_value}_weak.jsonl'), 'w') as f:
