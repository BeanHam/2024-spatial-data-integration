...
def main():
    ...
    args.save_path=f'inference_results/base/{args.model_id}_traveler/'
    ...
    configs=list(TRAVELER_INSTRUCTIONS.keys())
    ...
    for config in configs:
        ...
        def formatting_prompts_func(example):
            output = ""
            if config in ['zero_shot_with_heur_value_all_traveler', 'few_shot_with_heur_value_all_traveler']:
        ...
        base_instruction=TRAVELER_INSTRUCTIONS[config]
        test = data['test'].select(range(50)).map(formatting_prompts_func)
